  ...
  let user = firebase.auth().currentUser;
    user.sendEmailVerification().then(function(){
      swal("Correo de verificacion enviado, revisa tu correo para confirmarlo, en caso de no llegar da click sobre el boton reenviar", {
        icon: 'success',
        title: 'ACADEMICO & PERSONAL CUCEI-SRG',
...
*/
let redirect = () => {
  window.location.replace(`${_URL_}/validator-cucei.php`);
}
/*
...
let userLogIn = () => {
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
        ...
        validatedEmail(emailVerificado);
          ...
          success: function(data){
            $.each(data,function(key, registro) {
              ...
              if (codigo == 1) {
                window.location.replace(`${_URL_}/registro-datos-cucei.php`);
              }else{
                ...
                // }
                if(status === '3'){
                    window.location.replace(`${_URL_}/dashboard-cucei.php`);
                }
              }
            });
          },
      ...
      window.location.replace(`${_URL_}/ingreso-cucei.php`);
    }
  });
}
/*
